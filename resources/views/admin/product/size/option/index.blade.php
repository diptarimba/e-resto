...
<x-layoutContent
    ...
    mainTitle="Product Size"
    subTitle="Home"
>
    <x-card.card>
        <x-slot name="header">
            <x-card.card-title-create url="{{route('admin.product.size.option.create', ['product' => $product->id, 'size' => $size->id])}}" text="List"/>
        </x-slot>
        <x-slot name="body">
...
<script>
    $(document).ready(() => {
        ...
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.product.size.option.index', ['product' => $product->id, 'size' => $size->id]) }}",
        columns: [
