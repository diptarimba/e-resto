                    ...
                    <swiper :modules="[Pagination]" :pagination="{ clickable: true, hide: false }" :speed="1500" :slides-per-view="1" :observer="true" :observe-parents="true" :space-between="5" watch-slides-progress :loop="true">
                        <swiper-slide>
                            <div class="product-gallery-single-item">
                                <div class="image">
                                    <img class="img-fluid" width="276" height="172" src="/assets/images/product/single/product-gallery-single-1.png" alt="">
        ...
        <div class="container px-0">
            <div class="product-gallery-details">
                    ...
                    <li class="list-item" v-for="(sizeValue, sizeKey) in product.size" :key="sizeKey">
                        ...
                        <div class="right">
                            <ul class="size-chart inner-child-item">
                                <li v-for="(optionValue, optionKey) in sizeValue.size_option" :key="optionKey">
                                    <label :for="optionValue.id">
                                        <input type="radio" @click="chooseOption(sizeValue, optionValue)" :name="`size[${sizeValue.id}]`" :id="optionValue.id">
                                        <span class="size-text">{{optionValue.name}}</span>
                                    </label>
                                </li>
                            </ul>
                        </div>
                ...
                <div class="price-n-cart">
                    ...
                    <button class="btn cart" :disabled="optionChoosen.value" @click="addToCart">
    ...
    */
    ...
    dataFind = indexProductInCart.map((eachCart, keyCart) => {
        ...
        let eachProductOption = productAdd.option.map((eachProduct) => {
            ...
            let optionCheck = eachCart.option.findIndex(({
                key,
                value
            }) => {

                // perbandingan 1 per 1 key & value
                let coreCheck = key == eachProduct.key && value == eachProduct.value
                return coreCheck
            })
    ...
    key === undefined ? currentCart.push(unProxy(productAdd)) : currentCart[key] = productAdd
