<template>
    <div class="single-product-item product-item--style-1" :class="'product-item--bg-' + bgColour">
        ...
        <img class="image" :class="product.status === 'SUSPEND' ? 'product_not_available' : ''" :src="product.image[0]"
            alt="image" />
        </Link>
        <div class="content">
            <Link :href="route('product.detail', product.id, false)">
            <div class="content--left">
                <ul class="review-star">
                    <li v-for="(each, key) in getStarIcons(product.star)" :key="key" class="items fill">
                        <i class="icon" :class="each"></i>
                    </li>
                </ul>
            ...
            <div class="content--right">
                <button @click="modifyWishlist" aria-label="Wishlist"
...
<script setup>
    ...
    const getStarIcons = (star) => {
        const icons = [];
        const fullStar = "icon icon-carce-star-full";
        const halfStar = "icon icon-carce-star-half";
